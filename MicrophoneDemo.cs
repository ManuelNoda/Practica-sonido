...
namespace Whisper.Samples
{
    ...
    public class MicrophoneDemo : MonoBehaviour
    {
        ...
        private async void OnRecordStop(AudioChunk recordedAudio)
        {
            ...
            var text = res.Result.ToLower(); // lo pasamos a minusculas para el regex
            if (printLanguage)
            ...
            DetectRegexText(text); // nueva función 
        }
        ...
        private void DetectRegexText(string text)
        {
            // Cambiar la espada a verde
            if (Regex.IsMatch(text, @"\bverde\b"))
            {
            ...
            // El guerrero se hace grande
            if (Regex.IsMatch(text, @"\bgrande\b"))
            {
                warrior.transform.localScale = new Vector3(2.0f, 2.0f, 2.0f);
            }
        }
    }
}